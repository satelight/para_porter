...
use library::common_variable::{
    OmoteUra, BARIGA_FOLDER_PATH, OMOTE_FOLDER_PATH, SERVER_PORT, URA_FOLDER_PATH,
};
...
pub fn see_my_folder() {
        ...
        Ok(dir) => {
            for d_result in dir {
                ...
                if file_name.contains(".txt") {
                    ...
                    let parse_file =
                        ParseParaFile::read_from_file_path(OMOTE_FOLDER_PATH, &file_name);
                    if let Some(sunpo) = parse_file.get_kijyu_sunpou() {
            ...
            for find_file in find_files.iter() {
                let menu_sentence = format!(
                    "{}: 寸法:{} ✕ {} ✕ {}",
                    find_file.file_path, find_file.naikei, find_file.gaikei, find_file.hutosa
                );
    ...
    return_and_clear_screen();
}

pub fn return_and_clear_screen() {
    let _ = Confirm::new()
...
pub async fn receive_friend_ips() -> Vec<SettingJson> {
    ...
    for handler in handlers {
        // 問い合わせたデータをvecで追加していく。
        if let Ok(serde_json) = handler.await {
            setting_jsons.push(serde_json)
        }
    }
...
pub async fn put_files_several_folder(selected_para_info: &ParaInfo) -> WriteResult {
    ...
    if bariga_dir_path.exists() {
        let bariga_shift_jis_file =
            ShiftjisFile::read_from_file_path(BARIGA_FOLDER_PATH, &bariga_file_name);
    ...
    if omote_dir_path.exists() {
        let bariga_shift_jis_file =
            ShiftjisFile::read_from_file_path(OMOTE_FOLDER_PATH, &hyomen_file_name);
    ...
    if ura_dir_path.exists() {
        let bariga_shift_jis_file =
            ShiftjisFile::read_from_file_path(URA_FOLDER_PATH, &hyomen_file_name);
