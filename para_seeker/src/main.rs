...
async fn search_other_machine() {
        ...
        true => {
            ...
            match option_selected_para_info {
                Some(selected_para_info) => {
                    let process_result =
                        service::put_files_several_folder(&selected_para_info).await;
                    println!(
                        "設備名:{}のファイルを基に以下に保存しました。",
                        selected_para_info.machine_name
                    );

                    if process_result.bariga {
                        println!(
                            "バリ画ファイル:{}/{}に保存しました。",
                            BARIGA_FOLDER_PATH, selected_para_info.bariga_file_name
                        );
                    } else {
                        println!(
                            "バリ画ファイル:{}/{}に保存に失敗しました。",
                    ...
                    if process_result.omote {
                        println!(
                            "表表面ファイル:{}/{}に保存しました。",
                            OMOTE_FOLDER_PATH, selected_para_info.hyomen_file_name
                        );
                    } else {
                        println!(
                            "表表面ファイル:{}/{}に保存に失敗しました。",
                    ...
                    if process_result.ura {
                        println!(
                            "裏表面ファイル:{}/{}に保存しました。",
                            URA_FOLDER_PATH, selected_para_info.hyomen_file_name
                        );
                    } else {
                        println!(
                            "裏表面ファイル:{}/{}に保存に失敗しました。",
                ...
                None => {
                    println!("{}のファイルが存在していません。", hinmoku_code);
                }
            }
        }
        false => {
    ...
    service::return_and_clear_screen();
}
...
pub async fn search_ips() {
    ...
    let mut setting_json = match setting_json_path.exists() {
        true => SettingJson::read(false),
        false => SettingJson::read(true),
    };
    ...
    println!(
        "IPアドレス収集完了しました。{:?}/{:?}をご確認ください。",
        SETTING_DIR_PATH, SETTING_JSON_PATH
    );
