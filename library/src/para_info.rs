...
impl ParaInfo {
    pub fn new(hinmoku_code:&str,machine_name:&str,address:&str) -> Self{
            ...
            for dir_result in read_dir {
                ...
                let file_name = dir_entry.file_name().to_str().unwrap().to_string().to_ascii_uppercase();
                match hyomen_target_hinmoku.find(&file_name){
                    Some(_) => {
                        hyomen_file_name = file_name;
                        break;
                    },
                    None => {},
                }
            ...
            if !hyomen_file_name.is_empty(){
                let hyomen_file = ShiftjisFile::new(BARIGA_FOLDER_PATH,&hyomen_file_name);
